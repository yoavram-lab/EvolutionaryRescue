...
And in line 497: ``As an example, we have parameterized our model using estimates three triple-negative breast cancer (TNBC) clones under drug therapy from the literature. 
...
The following sentences were already in the Discussion (line 480): ``In this scenario, aneuploidy provides two advantages. First, it delays the extinction of the population, providing more time for the appearance of the resistance mutation. 
...
We have rewritten the sentence about the mean detection time (line 455): ``We find that for small and intermediate-sized tumors the mean detection time is approximately equal to the mean recurrence time (i.e., $\tau_a^r\approx\tau_a^{M}$ for $N<N_m^*$).''
} 
...
We have added an explanation of ODE (line 382): ``In this case the dynamics of the number of mutant cells is deterministic and can therefore be modeled by a system of ordinary differential equations (ODE), which describe how the number of mutant cells changes over time by its time derivative (eq.~D2).''
} 
...
\textcolor{blue}{We have now added density-dependence for the sensitives (line 226). The results are unchanged.}

- methods: I got a 404 error for the source code link: \url{https://github.com/yoavram-lab/EvolutionaryRescue}
...
We now mention that eq. 2 (previously eq. 3) is an alternative form of an equation from Orr and Unckless 2008 (line 261).
} 
...
We have added the definition of $p_m$ in line 425: ``where $p_m$ is the probability that a lineage starting from a single mutant cell escapes stochastic extinction.''
} 
...
\textcolor{blue}{
The return time with new mutation from Orr $\&$ Unckless 2014 (eq. 23) can be written as:
\begin{align*}
...
\textcolor{blue}{We have added a paragraph in the Discussion (line 524) on the connections to Iwasa et al 2003 and Osmond et al 2020, which we think are the two closest parallels for the main results. 
...
We have aldo added the following text to the Discussion (line 497): ``As an example, we have parameterized our model using estimates from three triple-negative breast cancer (TNBC) clones under drug therapy. 
We find that TNBC clones are either in the first scenario, in which aneuploidy provides at partial or full resistance to the drug, or the third scenario, in which aneuploidy provides tolerance to the drug. 
...
\textcolor{blue}{
We have removed the misleading sentence ``aneuploidy will either not play any role in evolutionary rescue or will be the main driver of adaptation'', which is incorrect. Instead, we write (line 530): ``Our results suggest that identifying aneuploidies that are tolerant or stationary may be worthwhile, as these are also expected to enhance the probability of rescue (Figure 3) and extend the window of opportunity for rescue (Figure 6).''\\
...
\textcolor{blue}{The GitHub repository will become available when the paper is accepted and will include a README file that explains the repository structure.
Meanwhile, the source code for our analysis can be privately accessed here: \url{https://www.dropbox.com/scl/fi/hrvqdocsr67qur7k59wxw/StanaEtAlCode.zip?rlkey=n7bcr3usx5ym6q3s2qkotanhx}
}
...
\textcolor{blue}{Our focus is on mutations that confer resistance. We assume deleterious mutations can occur with equal rates in all genotypes and therefore would not change the dynamics. We now explain these assumptions in line 216: ``Note that when we refer to drug-sensitive cells, we include those cells that have any aneuploidy that does not affect fitness in the presence of the drug. Additionally, we focus on mutations that confer resistance, neglecting deleterious mutations (which are more common). We assume deleterious mutations and other aneuplidies occur at similar rates across genotypes and therefore neglect their effect on the dynamics.''
} 
...
\textcolor{blue}{
We agree with all of these points. We have revised the introduction and discussion accordingly, see lines 55-87, 524-532.
}
...
\textcolor{blue}{We have added a short explanation for ``tolerant'' and that it has other meanings in the literature (line 90). We also mention the equivalence to in ``sub/super/critical'' from Osmond et al 2020 (line 270).
