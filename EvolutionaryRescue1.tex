...
\begin{equation*}
m_t>\left\lfloor\frac{3\log10}{\log\left(\frac{\lambda_m}{\mu_m}\right)}\right\rfloor+1.
\end{equation*}
...
\end{align}
\end{subequations}
where $p_w$ is the probability that the lineage of a single wild-type cell avoids extinction, and the approximation $(1-p_w)\approx e^{-p_w}$ assumes that $p_w$ (but not $N p_w$) is small.

In the Appendix below, we use the theory of multi-type branching processes to find approximate expressions \cref{survprobwapprox1,survprobwinitial,scenario3} for $p_w$ in different regimes. Substituting these  into \cref{rescue_prob2}, we find that the rescue probability can be approximated by
...
\daniel{Need to explain intuitively what all this means.}
...
%\presc=1-\left(1-p_w\right)^N\approx 1-\e^{-Np_w}=1-\exp\left[\frac{N}{\Delta_w}\left(\frac{u\Delta_a}{\lambda_a}+\frac{uv\Delta_m}{\lambda_m\Delta_a}+\frac{v\Delta_m}{\lambda_m}-\frac{uv^2\lambda_a\Delta_m^2}{8\lambda_m^2\Delta_a^3}\right)\right].
%\end{align}
%However, both approximations seem to work very well (\Cref{compare_1st_2nd_order_approx}).

These approximations accurately match full stochastic simulations (\Cref{rescue_prob_wt_growth,rescue_prob_an_growth,rescue_prob_N,rescue_prob_N}).
...
In this scenario, the probability $\psgv$ of evolutionary rescue via standing genetic variation, that is, by cells with aneuploidy from the initial population is:
\begin{equation*}
% redefine: p_total, p_denovo=p_resuce from eq above; p_stdvar is the new thing
\psgv = 1-\left(1-p_a\right)^{fN}\approx 1-\e^{-fNp_a}. 
\end{equation*}
Here $p_a$ is the probability that a lineage started by an aneuploid cell avoids extinction; see (EQUATION) in the Appendix for its value.
The total probability of evolutionary rescue is given by
\begin{align}\nonumber
\presc 	&= \psgv+\left(1-\psgv\right)p_\text{de novo} \\
			&= 1-\exp\left(-\left[\left(1-f\right)p_w + fp_a\right]N\right) .
\end{align}

The fraction of cases in which the population is rescued by pre-existing aneuploid cells (i.e., standing genetic variation) is given by $F\left(f\right)=\frac{\psgv}{p_\text{total}}$ (\Cref{FractionPlot}).
...
4\lambda_avp_m \gg \left(\Delta_a-v\right)^2 ,\\
...
\Delta_a < 0 \quad \text{and} \quad 4\lambda_avp_m \ll \left(\Delta_a-v\right)^2 ,\\
...
\Delta_a>0 \quad \text{and} \quad 4\lambda_avp_m \ll \left(\Delta_a-v\right)^2. % TODO this term can be simplified?
  \end{cases}
\end{equation}
\daniel{These expressions are impenetrable. Need to be simplified and interpreted.}

Aneuploidy always increases the probability of rescue ($H > 1$). 
We find that the rescue ratio increase with the aneuploidy growth rate $\Delta_a$, because the better aneuploid cells are in growth, the better they are at directly rescuing the population (when they provide partial resistance) or avoiding extinction long enough to acquire full resistance (when they provide tolerance). 
...
If the tumor is sufficiently large, then even in the absence of aneuploidy rescue is almost certain to occur, and therefore aneuploidy has only a negligible additional effect. % TODO can we find this analytical?

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

\subsection*{Evolutionary rescue time}

Even if evolutionary rescue occurs, it may take a long time; therefore, it is crucial to estimate the mean waiting time for rescue and the effect aneuploidy may have on it. 
...
\daniel{Need to be totally clear here that we're conditioning on rescue.}
...
\frac{\int_{0}^{\infty}{tR(t)\exp\left(-\int_0^{t}{R(z)\d z}\right) \d t}}{\presc} = 
\frac{\int_{0}^{\infty}{\exp\left(-\int_0^{t}{R(z)\d z}\right) \d t}}{\presc}
\end{aligned}
\end{equation}
...
When aneuploid cells are partially resistant to the drug ($\Delta_w\ll$(SOMETHING $\ne0$)$\ll\Delta_a\ll\Delta_m$), evolutionary rescue can be approximated by a one-step evolutionary rescue process where aneuploidy itself rescues the population (\Cref{rescue_prob_wt_growth}). 
When aneuploidy only provides tolerance to the drug ($\Delta_w\ll\Delta_a\ll$(SOMETHING $\ne0$)$\ll\Delta_m$), it cannot rescue the population.
...
\subsection*{Calculation of survival probabilities of single lineages}

Here we calculate the \emph{survival probability}, the probability that a lineage descended from a single cell does not become extinct.
Let $p_w$, $p_a$, and $p_m$ be the survival probabilities of a lineage started by a single wildtype cell, aneuploid cell, or mutant cell, respectively.
We will neglect density dependence, allowing us to treat the lineages as \emph{multitype branching processes} \citep{harris1963theory}.
The extinction probabilities $1-p_w$, $1-p_a$, and $1-p_m$ then satisfy the equations:
...
\daniel{Rewrite the below in terms of $\Delta k$.}

The survival probabilities are given by the smallest solution for each quadratic equation \citep{weissman2009rate,uecker2015adaptive}. Therefore we have
...
\begin{center}
  \begin{tabular}{| l |p{5cm}| c | c | p{3cm} |}
    ...
    $\Delta_k \equiv \lambda_k - \mu_k$ & 1/days & Growth rate of type $k$ cells, for $k=w,a,m$\\ \hline
...
Solid and dashed lines apply $\presc$ from the exact formula of  (\cref{survival_prob} in \cref{rescue_prob}); dashed lines apply $\presc$ from the approximate formula (\cref{rescue_prob_approx}), with good agreement.
...
%\presc=1-\e^{-\left(\theta_1+\theta_2\right)},
%\end{equation}
