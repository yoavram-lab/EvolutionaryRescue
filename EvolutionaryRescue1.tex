...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{abstract}
Evolutionary rescue is the process by which a population is able to survive a sudden environmental change which initially causes the population to decline towards extinction. A prime example of evolutionary rescue is the ability of cancer to survive being exposed to various treatments. Chromosomal instability (aneuploidy) occurs frequently in cancer cells and introduces large genetics, suggesting that it may be an important mechanism by which populations of cancer cells to adapt to chemotherapy. To determine when this should be the case, we model a population of cancer cells exposed to chemotherapy that can achieve evolutionary rescue with or without aneuploidy. We use branching process approximations to obtain simple expressions for the probability of evolutionary rescue and the effect of aneuploidy. We find that even when aneuploidy by itself only confers tolerance to the drug, i.e., is insufficient for rescue, it can still be an important stepping stone on the way to evolutionary rescue, allowing even small tumors to produce enough genetic diversity to avoid extinction.
\end{abstract}
...
\daniel{RULE OF THUMB: EVERYTHING IN INTRO SHOULD LEAD TOWARD OUR QUESTION/WORK.}
...
The role of chromosomal instability (CIN) in the emergence of cancer has been studied extensively in the past decades \citep{michor2005can,christine2018understanding,nowak2002role,pavelka2010dr,komarova2003mutation,zhu2018cellular}. \daniel{SO WHAT, WHO CARES? SHOULD BE ABOUT WHETHER CIN IS IMPORTANT IN CANCER. (AND ANSWER BETTER BE YES, OR IF ANSWER IS NO OUR HOOK SHOULD BE THAT MAYBE EVEN IF IT IS IMPORTANT IN GENERAL ITS IMPORTANT IN RESISTANCE.)}
...
However, when taking into account that the appearance of aneuploidy requires a mutation to trigger CIN, the probability that CIN precedes tumorigenesis is highly unlikely~\citep{komarova2004optimal}.
...
\emph{Evolutionary rescue} is the process where the population acquires a trait that increases fitness in the new environment such that extinction is averted. \daniel{It is mathematically equivalent to the problem of crossing fitness valleys \citep{weissman2009rate,weissman2010rate}. CUT?}
...
\daniel{WHICH OF THESE ARE RELEVANT FOR CANCER? WHAT'S BEEN DONE IN THE CONTEXT OF CANCER? OR IS THERE EVEN ANY REASON WHY CANCER IS SPECIAL WHEN IT COMES TO EVOLUTIONARY RESCUE?}

Models of evolutionary rescue usually assume that the fitness of the wildtype and mutant are homogeneous in time. An exception was given by \citet{marrec2020adapt}, who modeled the fitness of the wildtype and mutant as time dependent. Evolutionary rescue in a quasi-periodic environment has been investigated by \citet{marrec2023evolutionary} with the observation that stochasticity increases the chance of extinction in comparison to the case with deterministic environmental fluctuations. Additionally, \citet{uecker2011fixation} investigated the probability of fixation of a beneficial mutation in a variable environment with arbitrary time-dependent selection coefficient and population size. \daniel{CUT ALL OF THE ABOVE IN THIS PAR? WE AREN'T REALLY DEALING WITH CHANGE IN TIME EITHER.}
Most models focus on the probability that at least one mutation rescues the population. How multiple mutations contribute to the survival of the population is less explored, but \citet{wilson2017soft} have shown that evolutionary rescue is significantly enhanced by soft selective sweeps when multiple mutations contribute. Additionally, including a range of possible alleles with different fitness which could avert extinction has been coupled with Fisher's geometric model to analyze the probability of evolutionary rescue \citep{wahl2023evolutionary,martin2007distributions,osmond2020genetic}.
...
We analyze three scenarios: in the first, aneuploid cells are partially resistant, $\Delta_m>\Delta_a>0$; in the second, aneuploid cells are tolerant, $0>\Delta_a>\Delta_w$ \citep[see][for the distinction between susceptible, resistant, and tolerant]{brauner2016distinguishing}; in the third, aneuploid cells are non-growing, stationary or "barely growing", that is, either slightly tolerant or slightly resistant, such that $\Delta_a \approx 0$, in a sense that we will make more precise below. We assume that both chromosomal missegregation and mutation occur during the process of mitosis. Wildtype cells may divide and then missegregate to become aneuploids at rate $u\lambda_w$. Both aneuploid and wildtype cells may divide and mutate to become mutants at rates $v\lambda_{a}$ and $v\lambda_{w}$, respectively, which we assume is lower than the division rates, $v\cdot\max{(\lambda_w, \lambda_a)}<\min{(\lambda_w, \lambda_a, \lambda_m)}$.
See \Cref{figureAneuploidy} for a schematic representation of the model and \Cref{sampleTrajectories} for sample trajectories of the different genotypes for \textbf{(A)} $u=0$ (no aneuploidy), \textbf{(B)} $\Delta_a < 0$, \textbf{(C)} $\Delta_a\approx0$  and \textbf{(D)} $\Delta_a > 0$.
...
\daniel{SAY MORE ABOUT NATURAL VALUES OF THE PARAMETERS, TABLE}
...
Aneuploidy may contribute to evolutionary rescue by either preventing population extinction directly (when aneuploidy confers resistance, $\Delta_a>0$) or by delaying extinction until resistant mutants can be establish (when $0>\Delta_a>\Delta_w$).
\daniel{We assume independence between clonal lineages starting from an initial population of $N$ wildtype cells. NEED TO EXPLAIN WHY THIS IS GOOD APPROXIMATION, NOT ASSUMPTION.} (We check the effect of density-dependent growth on our results below).
...
The rescue probability of the whole population is then:
...
1-e^{-N/N^*},
\end{equation}
where $N^* \equiv 1/p_w$ is the threshold tumor size above which evolutionary rescue is very likely.
Specifically, when $N \ll N^*$, the probability for evolutionary rescue is $\presc \approx N / N^* \ll 1$  and when $N > N^*$, it is $\presc \approx 1$. 
...
First, if aneuploidy is very rare ($u\lambda_a T^*< 1$), or if aneuploidy is both rare ($u\lambda_a < -\Delta_a$) and very sensitive to the drug ($\Delta_a T^* < -1$), then rescue will likely occur by a direct resistance mutation in a sensitive cell, such that 
...
% TODO PLUG IN REAL VALUES ok
\daniel{WHAT ABOUT ANEUPLOIDY VS NO-ANEUPLOIDY TAKING STANDING GENETIC VARIATION INTO ACCOUNT? THIS IS MOST IMPORTANT QUANTITY, RIGHT?}
...
\daniel{WHAT'S THE INTERESTING REGIME HERE? ALSO NEED TO MENTION THAT TIME IS EVEN A QUESTION EARLY IN THE MANUSCRIPT, LIKE IN THE INTRO}
...
\daniel{MAKE NOTATION CONSISTENT WITH PREVIOUS SECTION. BE CLEARER ABOUT CONDITIONING}
...
\daniel{THIS WHOLE TIME SECTION SEEMS LIKE A SKETCH, NOT A DRAFT. WHAT IS THE MESSAGE?}
...
In this scenario, the tumor can be ``evolutionarily rescued'', or escape extinction, via one of two paths. In the direct path, a sensitive cell acquires a mutation that confers resistance that allows it to rapidly grow. In the indirect path, a sensitive cell first becomes aneuploid, which diminishes the effect of the drug, and then a cell from that aneuploid lineage acquires a mutation that confers resistance (\Cref{figureAneuploidy}).
...
\daniel{Our results show that the probability of evolutionary rescue increases with the initial tumor size $N$, the sensitive growth rate $\Delta_w$, the mutation rate $v$, and the aneuploidy rate $u$.
DIDN'T NEED A MODEL FOR ANY OF THAT. ANYTHING MORE INTERESTING?}
...
We find that aneuploidy can have a significant effect on evolutionary rescue (\Cref{rescue_prob}). Aneuploidy is unlikely to contribute to evolutionary rescue in primary tumors in which the number of cells is large enough ($N>10^8$) for the appearance of resistant mutation directly in sensitive cells before these cells become extinct (\Cref{rescue_prob}).
However, aneuploidy can have a crucial role in evolutionary rescue of secondary tumors, in which the number of sensitive cells may be below the detection threshold of $\sim10^7$  \citep{bozic2013evolutionary}. \daniel{This can have an impact on the recurrence of cancer after the resection of the primary tumor  through secondary tumors which are too small to be detected and for which chemotherapy is employed to prevent  cancer relapse and are estimated to cause the majority of cancer-related deaths~\citep{chaffer2011perspective}. THIS IS A GOOD POINT!}
...
\daniel{WHAT ABOUT DISTRIBUTIONS OF RECURRENCE TIME? CAN RAISE THE QUESTION EVEN IF WE DON'T HAVE ANY RESULTS}

\daniel{Paradoxically, we observe from \Cref{MeanTimeGrowthAneuploidyPlot} that the mean time of a rescue mutation to appear is significantly shorter for the case when $u=0$ when compared to the case $u>0$, however this can be explained by the fact this mean time is conditioned on evolutionary rescue and, as a result, aneuploidy increase the \emph{window of opportunity} in which a rescue mutation could appear thus increasing the mean time as well (\Cref{sampleTrajectories}). BETTER TO JUST BE CLEAR AND AVOID PARADOXES IN THE FIRST PLACE}

\daniel{We hypothesized that presence of \emph{standing variation} 
--the existence of a subpopulation of aneuploid cancer cells before therapy begins--can facilitate evolutionary rescue by reducing the waiting time for the appearance of aneuploid cells. Indeed, we observe that even when a small fraction of the initial tumor is aneuploid, evolutionary rescue is more likely to occur through this existing standing variation, rather then through \emph{de novo} aneuploid cells (\Cref{rescue_denovo}). IF RESCUE PRIMARILY OCCURS THROUGH SGV, THAT SHOULD BE THE CASE WE FOCUS ON, WITH DE NOVO AS THE LITTLE EXTRA SECTION}

\daniel{We observe from \Cref{KaplanMeierfig} that the probability of the mutant cell population reaching size $N$ is approximately zero before time $\tau_2^r$ which is the recurrence time for the deterministic case. This can be explained as follows: in the deterministic case there is a sufficient number of lineages produced such that there exists a lineage where each descendant will only reproduce and not die; the time it takes for this lineage to reach $N$ is the lower bound for the time of all other lineages to reach $N$ and this time cannot be smaller then $\tau_2^r$ by definition. Given that for small values of $N$ we expect that at most a single lineage will rescue the tumor, this lineage cannot reach $N$ before $\tau_2^r$ for the deterministic case \cref{eq:t2det}. DOESN'T BELONG IN DISCUSSION. MAYBE IN RESULTS?}

\paragraph{Experimental future direction}
Our model's predictions may be tested by experiments \citep{martin2013probability}. \daniel{DO WE REALLY NEED A REFERENCE FOR THIS STATEMENT?} For example, to study the effects of initial tumor size on the probability of evolutionary rescue, a large culture mass can be propagated from a single cancer cell in permissive conditions and then diluted to a  range of starting tumor sizes. Afterwards, these tumors may be exposed to anti-cancer drugs that induces aneuploidy % TODO ref OK
...
\daniel{WHY DO THIS KIND OF WET SIMULATION? NO POINT. BETTER TO TRY TO EXTRACT MODEL PARAMETER VALUES FROM EXPERIMENTS AND THEN SEE IF MODEL PREDICTIONS FIT REAL PATIENT DATA}
...
% I THINK WE HAVE A GOOD ARGUMENT FOR WHY THIS ISN'T IMPORTANT

\paragraph{Conclusions \daniel{WOULD USUALLY GO AT BEGINNING OF DISCUSSION}}
...
\daniel{MANY PROBLEMS WITH REFERENCES}
...
\daniel{WHY ARE SO MANY REFERENCES INCOMPLETE? WHY STILL CITING BOZIC FOR VALUES? (GOOD IF REFERENCE WAS FOUND IN BOZIC, BUT UNNECESSARY IF REFERENCE CAME FROM BIONUMBERS)}
...
\daniel{FIGURES 1,2, 4, 5 : TEXT AND LABELS NEED TO BE MUCH BIGGER.}
...
\caption{\textbf{Aneuploidy is a key driver of cancer adaptation.} \daniel{NO, THIS IS MESSAGE OF EARLIER FIG.}
...
\caption{Plot of the probability that a mutant cancer cell population has not reached size $N$ at time $t$. \daniel{WHY IS THIS SEPARATE FROM FIG 6?} The green line represents the case where $N=10^6$, the red line represents $N=10^7$ and the blue line represents the case where $N=10^{10}$. Increasing the initial tumor size guarantees that the tumor recurrence. The markers represent stochastic simulations and the error bars represents $95\%$ confidence interval of the form $p\pm1.96\sqrt{p\left(1-p\right)/n}$ where $p$ is the probability of that a mutant population size has not reached $N$ and $n=100$ is the number of simulations.
Parameters: $\lambda_w=0.1,\lambda_a=0.0899,\lambda_m=0.1,\mu_w=0.14,\mu_a=0.09,\mu_m=0.09, u=10^{-2}, v=10^{-7}$.}
