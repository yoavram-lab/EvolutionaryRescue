...
Here, division rate $\lambda_w=\lambda_a=\lambda_m=0.14$, (so that growth rate changes due to variable death rate), mutant death rate $\mu_m=0.13$ (so that mutant growth rate $\Delta_m=0.01$), aneuploidy rate $u=10^{-2}$, mutation rate $v=10^{-7}$.
}
