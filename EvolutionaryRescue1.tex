...
\end{aligned}
\end{equation}
Note that that \cref{eq:N_a} is an approximation of the two step-evolutionary rescue probability derived by \citet{osmond_genetic_2020} with Dirac delta fitness distribution, which is also similar to eq.~A3 in \citep{iwasa2004evolutionary}. %change 57
...
In addition, drugs that significantly increase the division rate of sensitive cells, $\lambda_s$, but do not affect their death rate, $\mu_s$, decrease cancer recurrence times (conditioned on evolutionary rescue). %change 49
