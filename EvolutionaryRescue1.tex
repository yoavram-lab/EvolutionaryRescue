...
When a fraction $f\approx 1.36\%$ of the initial cancer cell population is aneuploid, the mean evolutionary rescue time (\cref{meantimet2SGV}; \Cref{SGVEvolutionaryRescueTimeComplete}. We note that standing genetic variation reduces the mean evolutionary rescue time only for intermediary sized tumors but not for large or small tumors. % WHAT is the purpose of this sentence?

In Appendix E we derive the probability that by time $t$ a successful mutant has been generated. \Cref{cdffig}A show the agreement between our formula and simulation results for the case when aneuploidy is present and when it is absent.  We observe that aneuploidy start to have an effect on adaptation for timescales greater then 100 days. % WHAT is the purpose of this sentence?
...
\tau_a^r \approx \begin{cases}
 ...
 N \ll N_a^* ,\\ 
  ...
  \end{cases}
\end{equation}
\Cref{proliferationFigure} shows the agreement between our approximations and simulations.
For small tumors ($Nt \ll 10^8$), the mean recurrence time can be approximated as the sum of the mean time for the first rescue mutation to appear and the mean time for the lineage generated by that mutation to reach size $N$. The mean recurrence time grows logarithmically with tumor size $N$ and is the same order of magnitude as the mean evolutionary rescue time. Increasing the mutant growth rate $\Delta_m$ leads to shorter recurrence times while increasing the wildtype and aneuploid growth rates, $\Delta_w$ and $\Delta_a$ respectively, increases the recurrence time.
For large tumors ($N \gg 10^8$), the dynamics of the number of mutant cells is deterministic and the mean recurrence time becomes constant.
...
% Remus the next paragraphs look like you wanted to find approximations so you did, you reference them here, but the results are buried in the technical text: WHAT IS IT THAT YOU WANT US TO LEARN from this? 
