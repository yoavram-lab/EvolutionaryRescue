...
% To me the big picture of the paper isn't clear.  I appreciate that the model can replicate what one expects, but what are the clinical implications, or surprising results?  Which of the scenarios are most realistic given the available clinical and experimental data?  What are the biological interpretations of the derived formulate 

% it's also not clear to me from the introduction that this is a novel model, which it is, no?

% Big picture:
% (1) Any time you say "we observe" or "interestingly" it sets up the expectation for biological interpretation - going through the paper and ensuring that such interpretation is present will help a lot
% (2) What's the overall story of the paper beyond we made a model and it does what we expect.  I'm not saying that there isn't more to the story here, I think there is, but it's not immediately clear to me what that more is.
% (3) adding interpretation to figure discussion/caption.  For example, in figure 3  (to me, but perhaps I'm wrong) it is unsurprising that the probability of rescue increases with population size, and that partially resistant saturates faster than stationary which is faster than partially tolerant.  It seems to me the main message of the figure right now is that the presence of aneuploidy in turmor cells, even if only partially tolerant to treatment, facilitates rescue. However, it seems like the more crucial message in the figure is the number of cells at which the curves saturates.  Perhaps one could plot the saturation level in terms of number of cancer cells as a function of the parameter value(s) which define each scenario. or perhaps some combination of the N*s and the saturation points 

% other comments:
% figures 2, 3 shouldn't have the same bolded caption 
% 

% some line item comments are below
\begin{abstract}
...
% It feels like conclusions are missing from this paragraph, we did these things and found what?...
% from the abstract "aneuploidy can play an important role in the relapse of secondary tumors who have not yet been detected and thus are smaller in size." is this not already suggested by the sources in paragraph two above?
% This paragraph should make clear the contribution of this paper, to me at the moment it's not...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section*{Methods}
...
See \Cref{figureAneuploidy} for a schematic representation of the model and \Cref{sampleTrajectories} for sample trajectories of the different genotypes for $u=0 \, (A), \Delta_a\ll0\, (B),\Delta_a\approx0\, (C),\Delta_a\gg0\, (D)$. We assume that wildtype cells acquire aneuploidy before the onset of therapy with rate $\tilde{u}\lambda_w$ and that aneuploid cells have a fitness cost $s$ in the drug-free environment.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
We set the mutation rate $v$ to be equal to $10^{-7}$ per gene per cell division \citep{loeb2001mutator} and since we assume that a single target gene confers immunity to chemotherapy we select $v=10^{-7}$ per cell division. % the above sentence feels repetitive 
...
\frac{N^*_a}{N^*_m} \approx \begin{cases}
  ...
  \end{cases}
\end{equation}
% the bracket here is meant to denote multiplication, approximation, what?
% Can you interpret this more directly in a biological sense?
...
\paragraph{Recurrence time.} % and? whats the further interpretation of this?
...
% I may need to reread, but what are the specific predictions in this paper that can be tested by experiments?
