...
We repeat these iterations until either the population becomes extinct (the number of cells of all types is zero) or the number of mutant cells is high enough so that its extinction probability is $<0.1\%$. % TODO how do you determine the extinction prob? ref to equation OK
...
\end{align}
\Cref{SurvPlot} for $N=10^4$ and \Cref{SurvPlotLargeN} for $N=10^8$  show that the survival probability, $p_{resc}$, quickly grows as the wildtype growth rate, $\lambda_w$, increases. % Todo I dont think this figure should show lambda_w>1, as in that case the wildtype is resistant and there is no rescue scenario. OK
...
\end{align}
\Cref{SurvPlotNData} show $p_{rescue}$ as a function of $N$, including comparison of our first approximation \eqref{aneuploidyresistentfirstapprox} % ref to equation OK
