...
\end{align}
which we plot in Figure \ref{MeanTimeGrowthAneuploidyPlot} as a function of the initial population size.
...
\end{align}
where $\erfc$ is the complementary error function. We plot the expansion \eqref{limitapprox2} in Figure \ref{MeanTimeGrowthAneuploidyPlot} and observe that it is a very good fit for large values of the initial wildtype population size.
...
Rescue probability is very high when aneuploidy provides partial resistance ($\lambda_a=0.01$), in an initially small tumor (\textbf{Aleft}, $N=10^4$) and even more so in an initially large tumor (\textbf{Aright}, $N=10^8$). % TODO change panel labels to ABCD
...
Here, aneuploidy rate is $u=10^{-2}$ and mutation rate is $v=10^{-7}$.
...
\textbf{(B)} Comparison of results of simulations  with density-dependent growth (markers with with 95\% CI) and the exact formula (blue line, \cref{survival_prob} in \cref{rescue_prob}). Here, $K=10^9$.
% TODO add parameter values
}
\label{rescue_prob_N}
\end{figure}
