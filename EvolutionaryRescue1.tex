...
\end{aligned}
\end{equation}
\Cref{eq:N_a} is our model's equivalent of \citet{iwasa2004evolutionary}'s eq.~A3, or eq.~8 of \citet{osmond_genetic_2020} with the distribution of fitness effects set to a delta function;
our ``tolerant'', ``stationary'', and ``resistant'' correspond to \citet{osmond_genetic_2020}'s ``sufficiently subcritical'', ``sufficiently critical'', and ``sufficiently supercritical'', respectively. %change 57
...
Instead, it acts as a ``stepping stone'' through which the resistant mutant can appear more rapidly, given that the number of aneuploid cells declines slower than the number of drug-sensitive cells (\Cref{sampleTrajectories}). 
...
Comparing our results to the evolutionary rescue literature, we complement \citet{iwasa2003evolutionary} and \citet{osmond_genetic_2020}'s work on multi-step rescue by deriving simple approximations for a different fitness landscape. 
Similar to \citet{osmond_genetic_2020}, we find that two-step rescues can be more likely than simple one-step rescues. 
The situation in which the intermediate growth rate is close to zero in the presence of drug (``stationary'') is particularly interesting.
While in a general model of evolutionary rescue this might appear to be only a tiny region of parameter space 
(Fig.~\ref{rescue_threshold}A, inset),
the example of some aneuploidies in cancer suggests that it may be biologically relevant. 
