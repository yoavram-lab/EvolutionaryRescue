...
We find that ... (\Cref{rescue_prob_N_logistic}). % TODO check what we find; the figure just shows agreement of exact and stochastic
...
p_{old} = 1-\left(1-p_a\right)^{fN}\approx 1-\e^{-fNp_a}. % TODO should be p_new?
\end{equation*}
The total probability of evolutionary rescue is given by
\begin{align}\nonumber
p_{total} 	&= p_{new}+\left(1-p_{new}\right)p_{old}\\ % TODO what is p_new?
...
\begin{equation}\label{halfeqstandvar} % TODO not sure this is interesting
...
However, the rescue decreases with the wildtype growth rate $\Delta_w$, because the better the wildtype is at growth, the less is depends on aneuploidy for rescue or delay, and the more likely it is to directly produce mutant cells, rather than relying on aneuploid cells for producing mutant cells (\Cref{RatioEvolRescue}).
The effect of the initial tumor size $N$ is mostly the same as the wildtype growth rate, except when aneuploidy provides tolerance and the population size is very large, in which case the ratio ... % TODO does H go to 1 in this case? is it lower than 1 for delta_a=-0.001? why?
...
In aneuploid cells already exist in the population at the onset of drug therapy as standing genetic variation, then evolutionary rescue is more likely... % TODO think about this figure...
...
Solid and dashed lines apply $p_{rescue}$ from the exact formula of  (\cref{survival_prob} in \cref{rescue_prob}); % TODO do you use 1-(1-pw)^N or 1-e(-Np_w)?
