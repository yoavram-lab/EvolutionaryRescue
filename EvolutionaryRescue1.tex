...
To model standing genetic variation, we assume that before the onset of therapy, wildtype cells become aneuploid with rate $\tilde{u}\lambda_w$ (which may differ from $u \lambda_w$) and that aneuploidy confers a fitness cost $s$ in the drug-free environment (we assume that aneuploidy significantly increases the death rate of wildtype cancer cells in an environment which is not affected by chemotherapeutic drugs). % TODO unclear how s affects \lambda_w; I do not think that s has any effect on \lambda_w as s is the fitness of the aneuploid in the drug free environment
...
\citet{bakker2023predicting} determined that the missegregation rate must be between $10^{-3}-10^{-2}$ per chromosome per cell division. Given that a duplication is beneficial for cancer resistance only in one or two chromosomes (see details next paragraph) we use $u=10^{-2}$ per cell division.
...
\daniel{THIS DERIVATION DOESN'T MAKE SENSE. FROM THE LUKOW EXPERIMENT WE CAN DIRECTLY ESTIMATE THE FITNESS COST AS $s \approx \ln(0.5/0.15)/24 \approx .05$ PER DAY. DOING IT MORE CAREFULLY (LOGISTIC RATHER THAN EXPONENTIAL) GIVES ABOUT THE SAME ANSWER.}
...
In the second case, aneuploid cells are sufficiently resistant that the expected size of each aneuploid lineage is roughly 1.  % TODO that's a weird sentence; a lineage starts with a single cell, if the lineage size stays constant that it stays one. so what are we talking about? try to rephrase. ok
...
\end{equation}
Comparing the above approximation to results of stochastic evolutionary simulations, we find that the approximations perform very well (\Cref{rescue_denovo}).  % TODO which approximations? unclear ok
...
For example, with  $\lambda_w=0.1,\mu_w=0.14, u=10^{-2}, \tilde{u}=10^{-3}, s=7.3\times10^{-3}$, the ratio of the threshold tumor sizes for standing vs de-novo variation is $\tilde{N}^*_a/N^*_a \approx 1.825$, which means that de-novo genetic variation is  the main driver of evolutionary rescue.
...
However, when the expected number of resistant lineages is large, the behavior of the size of the population of mutant cells is deterministic (i.e. it is modeled by a system of ODEs, see \Cref{detODE}) and the mean recurrence time cannot be separated into the mean evolutionary rescue time and mean proliferation time because multiple mutant lineages contribute towards the mutant population size reaching the initial tumor size. % TODO I dont like the word "behaves", please be specific. - what exactly is deterministic and why does it mean you cannot do the separation. ok
...
\begin{center}
  \begin{tabular}{| l |p{5cm}| c | c | p{3cm} |}
    ...
    $s$ & Selection coefficient of aneuploidy in the drug free environment& $7.3 \times 10^{-3}$ &  1/days   & \citet{lukow2021chromosomal} \\  
    \hline
  \end{tabular}
