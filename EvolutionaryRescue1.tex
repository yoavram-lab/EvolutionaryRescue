...
\Cref{MeanTimeGrowthAneuploidyPlot}A shows that the approximation \cref{limitapprox3} has a good fit with simulation results for large initial wildtype population size ($N > 2\cdot10^7$). An approximation that uses second-order terms $\frac{\Delta_w^2\tau^2}{2}$ and $\frac{\Delta_a^2\tau^2}{2}$ does not perform better (\Cref{MeanTimeGrowthAneuploidyPlot}A).
