...
We define tolerant cells as those that survive drug exposure longer than susceptible cells, while resistant cells require higher drug concentrations to be affected. For details on these distinctions, see \citet{brauner2016distinguishing}.
...
Note that when we refer to drug-sensitive cells, we include those cells that have any aneuploidy that does not affect fitness in the presence of the drug (\Cref{table1}). %change S7
...
Using \cref{eq:N_a,eq:N_m,eq:Nsgv}, we can find the ratio of threshold tumor size for rescue via standing genetic variation  to the threshold for rescue via \textit{de novo} genetic variation,
