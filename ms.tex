...
\section{Recurrence time}\label{sec:appendix_recurrence_time} % TODO it is not clear what are the assumptions here; reading the main text and then coming here I think that you assume tolerant aneuploidy - is that the case? you need to be EXPLICIT
...
\end{equation*}
where we ignore lineages created by direct mutation because we assumed $u\lambda_a > \max{(-\Delta_a, 1/T^*)}$, $N\ll N_m^*$ and used \cref{eq:N_a}. % TODO Remus why did you assume these assumptions? say the biological assumption, not just the math. and how did you use eq 3? write it EXPLICITLY.
...
\tau_a^r&\approx\tau_a+\frac{\log p_mN}{\Delta_m}. % TODO is it log(pm) N or log(pm N) ? add parentheses.
\end{align}
The factor of $p_m$ in the second term of \cref{meanproliferationtime} is due to the fact that the lineage is conditioned to survive genetic drift and the time to reach $N$ is shorter then the scenario without this property \citep{orr2014population,smith1974hitch}. % change 35 % TODO without what property? 
...
\end{equation}
This is a transcendental equation which cannot be solved exactly but we can obtain an approximation by noting that for large $\tau_a^r$ the above equation can be written as % TODO Remus what do you mean large tau? large compared to what? why do you assume it is large? what is the biological assumption? It is not clear why you neglect two out of three terms, and you need to be explicit about this -- say both what the math is but also what the biological assumption is. the editor  has asked for as much.
\begin{equation*}
