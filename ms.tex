...
Their results suggest that aneuploidy and other copy number alterations provide resistance to the drug cisplatin.  
Thus, we assume that aneuploid breast cancer cells are partially or even fully drug-resistant, i.e., $\Delta_m \ge \Delta_a > 0$.
...
We therefore have the following set of equations: $0.047 = \exp{(\Delta_{SA609}-\Delta_s)}, 0.02=\exp{(\Delta_{SA1035}-\Delta_s)}, 0.01=\exp{(\Delta_{SA535}-\Delta_s)}$.
...
As all three clones grow in the presence of the drug, we set the growth rate of the least fit, TNBC-SA535 clone H, to the growth rate of breast cancer cells in the absence of the drug, $\Delta_{SA535}=0.0085$, and then solve the set of equations. % TODO needs to be the most fit, wait for Remus
We further assume the drug affects death rather than division. We therefore have $\lambda_s=\lambda_a=0.0845$ for all clones,  $\mu_{SA609} = 0.0415$, $\mu_{SA1035}=0.0667$, $\mu_{SA535}=0.085$, and $\mu_s=0.085$.
...
In our parameterization for melanoma and breast cancer, the value of $\Delta_a T^*$ is over 500.
Therefore, in these cases, we are in the third scenario, in which aneuploidy provides resistance to the drug. % change b
...
As an example, we have parameterized our model using estimates for melanoma and triple-negative breast cancer (TNBC) cells under drug therapy from the literature. 
...
We have also compared the probability of evolutionary rescue between melanoma and TNBC (black and color lines, respectively, in \Cref{rescue_prob}B).
...
Our results suggest that identifying aneuploidies that are tolerant or stationary may be worthwhile, as these are also expected to enhance the probability of rescue (\Cref{rescue_prob}) and extend the window of opportunity for rescue (\Cref{cdffig}).
...
\begin{center}
  \begin{tabular}{| l |p{5cm}| c | c | p{3cm} |}
    ...
    $\lambda_s$ & Sensitive division rate& 0.0848& 1/days \\ \hline
    $\mu_s$ & Sensitive death rate& $0.085$ & 1/days   \\ \hline
    $\lambda_a$  & Aneuploid division rate & $0.0845$ & 1/days    \\ \hline
    $\mu_a$ & Aneuploid death rate & $0.0415$, $0.0667$, $0.085$ & 1/days   \\ \hline
  \end{tabular}
...
\textbf{(B)} Comparison for melanoma (black) and breast cancer (colors)  with parameters from the literature (\Cref{table1,table2}). Black line: Melanoma A375. Blue line: Breast cancer TNBC-SA1035 clone H. Red line: Breast cancer TNBC-SA609 clone A. Green line: Breast cancer TNBC-SA535 clone H.
}
\label{rescue_prob}
\end{figure}
...
This helps us distinguish between two scenarios for the proliferation time. Firstly, when we have at most one lineage which rescues the cancer cell population:
\begin{equation*}
N\ll N_a^*.
\end{equation*}
...
\end{equation}
We obtain $\tau_a^r$ by solving \eqref{m_t} for the time at which the number of mutant cells reaches $N$, i.e., $m\left(\tau_a^r\right)=N$:
...
\end{equation}
We observe that the terms given by the evolutionary trajectory \textit{sensitive} $\rightarrow$ \textit{aneuploid} $\rightarrow$ \textit{mutant} do not contribute to the above approximation and, as a result, we deduce that it is accurate only for $N \gg N_m^* > N_a^*$. %change 9
