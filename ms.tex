...
\section{Recurrence time}\label{sec:appendix_recurrence_time} % TODO it is not clear what are the assumptions here; reading the main text and then coming here I think that you assume tolerant aneuploidy - is that the case? you need to be EXPLICIT OK
For this section we assume that aneuploids are tolerant and we define the proliferation time $\tau_a^p$  to be the expected time for the number of resistant cells to grow to the initial tumor size $N$.
...
\end{equation*}
where we assumed that aneuploidy is frequent enough to affect the evolution of drug resistance (i.e. $u\lambda_a > \max{(-\Delta_a, 1/T^*)}$). Additionally, we have used \cref{eq:N_a,eq:N_m} to write $N_a^*=\frac{\abs{\Delta_s}\abs{\Delta_a}}{uv\lambda_s\lambda_ap_m}$ and $N_m^*=\frac{\abs{\Delta_s}}{v\lambda_sp_m}$. % TODO Remus why did you assume these assumptions? say the biological assumption, not just the math. and how did you use eq 3? write it EXPLICITLY. OK

This helps us distinguish between two scenarios for the proliferation time. 
In the first scenario, $N\ll N_a^*\ll N_m^*$, and we have at most one lineage which rescues the cancer cell population.
As a result, the recurrence time is given by~\citep{avanzini2019cancer}:
\begin{align}\label{meanproliferationtime}
\tau_a^r&\approx\tau_a+\frac{\log \left(p_mN\right)}{\Delta_m}. % TODO is it log(pm) N or log(pm N) ? add parentheses. OK
\end{align}
The factor of $p_m$ in the second term of \cref{meanproliferationtime} is due to the fact that the lineage is conditioned to survive genetic drift and the time to reach $N$ is shorter then the scenario when the lineage is not conditioned to survive stochastic extinction~\citep{orr2014population,smith1974hitch}. % change 35 % TODO without what property? OK

In the second scenario, $N\gg N_m^*$, and the sensitive population produces a large number of rescue lineages in a short period of time.
...
\end{equation}
This is a transcendental equation which cannot be solved exactly but we can obtain an approximation by noting that $\e^{\Delta_m \tau_a^r} \gg\e^{\Delta_s \tau_a^r},\e^{\Delta_a \tau_a^r}$ for even small values of $\tau_a^r$ because $\Delta_m>0$ and $\Delta_s,\Delta_a<0$. Given we assumed that the tumor is large (i.e. $N\gg N_a^*$) then the time $\tau_a^r$ necessary for the resistant cell population to reach $N$ is large enough such that the first term of \cref{eq:t2det2024} can be written as: % TODO Remus what do you mean large tau? large compared to what? why do you assume it is large? what is the biological assumption? It is not clear why you neglect two out of three terms, and you need to be explicit about this -- say both what the math is but also what the biological assumption is. the editor  has asked for as much. OK
\begin{align*}
...
\end{align*}
which is significantly smaller then the second term of \cref{eq:t2det2024}:
\begin{align*}
...
\end{align*}
Consequently, \cref{eq:t2det2024} can be approximated as:
\begin{equation*}
