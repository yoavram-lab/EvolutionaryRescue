...
In the following, we assume aneuploid cells are tolerant ($\Delta_a T^* \ll 1$) and frequent enough to affect the evolution of drug resistance (i.e. $u\lambda_a > \max{(-\Delta_a, 1/T^*)}$). 
...
We distinguish between two scenarios for the proliferation time. 
In the first scenario, $N\ll N_a^*\ll N_m^*$, and we have at most one lineage that rescues the cancer cell population.
...
\end{equation}
This is a transcendental equation which cannot be solved exactly but we can obtain an approximation by noting that $\e^{\Delta_m \tau_a^r} \gg\e^{\Delta_s \tau_a^r},\e^{\Delta_a \tau_a^r}$ for even small values of $\tau_a^r$ because $\Delta_m>0$ and $\Delta_s,\Delta_a<0$. 
We assume that the tumor is large ($N\gg N_m^*$) % REMUS you assumed N\gg N_m^* not N\gg N_a^* FIXED
so the time $\tau_a^r$ for the resistant cell population to reach $N$ is large enough such that the first term of \cref{eq:t2det2024} can be written as
\begin{align*}
...
\end{align*}
Consequently, \cref{eq:t2det2024} can be approximated by
\begin{equation*}
