...
txt='DataGenerationFractionAneuploidyratiomutations.txt'
file = open(txt, "w")
...
for u in [10**(-3),3*10**(-3),10**(-2),3*10**(-2),10**(-1)]:
    ...
    while n < reps:
        Sum2.append(gillespie_ssa(λs, λa, λm, μs, μa, μm, u, v, s0=0, a0=N2, tmax=T))
        n += 1
    ...
    while n < reps:
        Sum1.append(gillespie_ssa(λs, λa, λm, μs, μa, μm, u, v, s0=N1, a0=0, tmax=T))
        n += 1
    ...
    file.write(str(us/u)+' '+str(m1/m2)+' '+str(m1/m2-np.percentile(M, 2.5))+' '+str(np.percentile(M, 92.5)-m1/m2)+' ')
