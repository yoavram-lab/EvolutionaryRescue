...
λw, λa, λm, μw, μa, μm, v = 0.1, 0.0899, 0.1, 0.14, 0.09, 0.09, 10**(-7)
s, us = 7.3*10**(-3), 10**(-3)
...
N = [U/us*(abs(s)/abs(λw - μw)) for U in u]
x = [us/U for U in u]
...
txt=str("DataGenerationFractionAneuploidyDanielratiomutations.txt")
...
ax.set_xscale('log')
ax.set_yscale('log')
#plt.yticks([10**(-6),5*10**(-6),10**(-5)], [r'$10^{-6}$',r'$5\times10^{-6}$',r'$10^{-5}$'],fontsize=14)
